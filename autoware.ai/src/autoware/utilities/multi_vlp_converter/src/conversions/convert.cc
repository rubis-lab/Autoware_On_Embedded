...
#include <multi_vlp_converter/pointcloudXYZIRT.h>
#include <multi_vlp_converter/organized_cloudXYZIRT.h>
...
namespace multi_vlp_converter
{
    ...
    diagnostics_(node, private_nh, node_name)
  {
    ...
    private_nh.param<std::string>("points_topic", config_.points_topic, "velodyne_points");
    private_nh.param<std::string>("packets_topic", config_.packets_topic, "velodyne_packets");
    ...
    boost::optional<multi_vlp_converter::Calibration> calibration = data_->setup(private_nh);
    if(calibration)
    {
    ...
    output_ =
      node.advertise<sensor_msgs::PointCloud2>(config_.points_topic, 10);

    srv_ = boost::make_shared <dynamic_reconfigure::Server<multi_vlp_converter::
      CloudNodeConfig> > (private_nh);
    dynamic_reconfigure::Server<multi_vlp_converter::CloudNodeConfig>::
      CallbackType f;
    ...
    velodyne_scan_ =
      node.subscribe(config_.packets_topic, 10,
    ...
    diag_topic_.reset(new TopicDiagnostic(config_.points_topic, diagnostics_,
  ...
  void Convert::callback(multi_vlp_converter::CloudNodeConfig &config,
                uint32_t level)
  {
