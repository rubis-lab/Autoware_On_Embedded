...
TrajectoryCost TrajectoryDynamicCosts::DoOneStepDynamic(const vector<vector<WayPoint>> &rollOuts, const vector<WayPoint> &totalPaths,
...
TrajectoryCost TrajectoryDynamicCosts::DoOneStepStatic(const vector<vector<WayPoint>> &rollOuts, const vector<WayPoint> &totalPaths,
                                                       const WayPoint &currState, PlanningParams &params, const CAR_BASIC_INFO &carInfo,
                                                       const VehicleState &vehicleState, const std::vector<PlannerHNS::DetectedObject> &obj_list,
    ...
    for (unsigned int ic = 0; ic < rollOuts.size(); ic++) {
        ...
        if (ic == currIndex && m_TrajectoryCosts.at(ic).bBlocked == true)
...
TrajectoryCost TrajectoryDynamicCosts::DoOneStep(const vector<vector<vector<WayPoint>>> &rollOuts, const vector<vector<WayPoint>> &totalPaths,
                                                 const WayPoint &currState, const int &currIndex, const int &currLaneIndex, PlanningParams &params,
...
void TrajectoryDynamicCosts::CalculateLateralAndLongitudinalCostsStatic(vector<TrajectoryCost> &trajectoryCosts,
                                                                        ...
                                                                        const WayPoint &currState, const vector<WayPoint> &contourPoints,
                                                                        PlanningParams &params, const CAR_BASIC_INFO &carInfo,
...
void TrajectoryDynamicCosts::CalculateLateralAndLongitudinalCosts(vector<TrajectoryCost> &trajectoryCosts,
                                                                  ...
                                                                  const vector<WayPoint> &contourPoints, PlanningParams &params,
                                                                  const CAR_BASIC_INFO &carInfo, const VehicleState &vehicleState) {
    double critical_lateral_distance = carInfo.width / 2.0 + params.horizontalSafetyDistancel;
    double critical_long_front_distance = carInfo.wheel_base / 2.0 + carInfo.length / 2.0 + params.verticalSafetyDistance;
    double critical_long_back_distance = carInfo.length / 2.0 + params.verticalSafetyDistance - carInfo.wheel_base / 2.0;
            ...
            for (unsigned int it = 0; it < rollOuts.at(il).size(); it++) {
                ...
                for (unsigned int icon = 0; icon < contourPoints.size(); icon++) {
                    ...
                    double direct_distance =
                        hypot(obj_info.perp_point.pos.y - contourPoints.at(icon).pos.y, obj_info.perp_point.pos.x - contourPoints.at(icon).pos.x);
                    if (contourPoints.at(icon).v < params.minSpeed && direct_distance > (m_LateralSkipDistance + contourPoints.at(icon).cost)) {
                        skip_id = contourPoints.at(icon).id;
                        continue;
                    }
                    ...
                    if (lateralDist <= critical_lateral_distance && longitudinalDist >= -carInfo.length / 1.5 &&
                        longitudinalDist < params.minFollowingDistance) {
                        trajectoryCosts.at(iCostIndex).bBlocked = true;
                    }
...
void TrajectoryDynamicCosts::NormalizeCosts(vector<TrajectoryCost> &trajectoryCosts, int enableDebug) {
    ...
    for (unsigned int ic = 0; ic < trajectoryCosts.size(); ic++) {
        ...
        trajectoryCosts.at(ic).cost =
            (m_WeightPriority * trajectoryCosts.at(ic).priority_cost + m_WeightTransition * trajectoryCosts.at(ic).transition_cost +
             m_WeightLat * trajectoryCosts.at(ic).lateral_cost + m_WeightLong * trajectoryCosts.at(ic).longitudinal_cost) /
            4.0;

        if (enableDebug) {
            std::cout << "Index: " << ic << ", Priority: " << trajectoryCosts.at(ic).priority_cost
                      << ", Transition: " << trajectoryCosts.at(ic).transition_cost << ", Lat: " << trajectoryCosts.at(ic).lateral_cost
                      << ", Long: " << trajectoryCosts.at(ic).longitudinal_cost << ", Change: " << trajectoryCosts.at(ic).lane_change_cost
                      << ", Avg: " << trajectoryCosts.at(ic).cost << ", Blocked : " << trajectoryCosts.at(ic).bBlocked << std::endl;
        }
    }
    if (enableDebug)
        std::cout << "------------------------ " << std::endl;
...
vector<TrajectoryCost> TrajectoryDynamicCosts::CalculatePriorityAndLaneChangeCosts(const vector<vector<WayPoint>> &laneRollOuts,
                                                                                   const int &lane_index, PlanningParams &params) {
    ...
    for (unsigned int it = 0; it < laneRollOuts.size(); it++) {
        ...
        //    else if(laneRollOuts.at(it).at(0).bDir == BACKWARD_DIR || laneRollOuts.at(it).at(0).bDir == BACKWARD_RIGHT_DIR ||
...
void TrajectoryDynamicCosts::CalculateTransitionCosts(vector<TrajectoryCost> &trajectoryCosts, const int &currTrajectoryIndex,
...
void TrajectoryDynamicCosts::CalculateIntersectionVelocities(const std::vector<PlannerHNS::WayPoint> &path, const PlannerHNS::DetectedObject &obj,
            ...
            for (unsigned int i = 0; i < path.size(); i++) {
                // if(path.at(i).timeCost > -1)
                {
                    double collision_distance =
                        hypot(path.at(i).pos.x - obj.predTrajectories.at(k).at(j).pos.x, path.at(i).pos.y - obj.predTrajectories.at(k).at(j).pos.y);
                    ...
                    if (collision_distance <= c_lateral_d && i < closest_path_i) {
                        ...
                        closest_path_i = i;
                        double a =
                            UtilityHNS::UtilityH::AngleBetweenTwoAnglesPositive(path.at(i).pos.a, obj.predTrajectories.at(k).at(j).pos.a) / M_PI;
...
int TrajectoryDynamicCosts::GetCurrentRollOutIndex(const std::vector<WayPoint> &path, const WayPoint &currState, PlanningParams &params) {
...
void TrajectoryDynamicCosts::InitializeCosts(const vector<vector<WayPoint>> &rollOuts, PlanningParams &params) {
    m_TrajectoryCosts.clear();
    if (rollOuts.size() > 0) {
        TrajectoryCost tc;
        int centralIndex = params.rollOutNumber / 2;
...
void TrajectoryDynamicCosts::InitializeSafetyPolygon(const WayPoint &currState, const CAR_BASIC_INFO &carInfo, const VehicleState &vehicleState,
...
void TrajectoryDynamicCosts::CalculateLateralAndLongitudinalCostsDynamic(const std::vector<PlannerHNS::DetectedObject> &obj_list,
                                                                         const vector<vector<WayPoint>> &rollOuts, const vector<WayPoint> &totalPaths,
                                                                         const WayPoint &currState, PlanningParams &params,
                                                                         const CAR_BASIC_INFO &carInfo, const VehicleState &vehicleState,
                                                                         const double &c_lateral_d, const double &c_long_front_d,
