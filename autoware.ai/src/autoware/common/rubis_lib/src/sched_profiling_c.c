...
void init_task_profiling(char *_task_response_time_filename) {
    ...
    fprintf(task_response_time_fp_,
            "iter,PID,start,end,instance,lidar_instance,vision_instance\n");
}
...
void stop_task_profiling(unsigned long instance, unsigned long lidar_instance_,
    ...
    fprintf(task_response_time_fp_, "%d,%d,%lld.%.9ld,%lld.%.9ld,%lu,%lu,%lu\n",
            iter_++, getpid(), (long long)task_start_time_.tv_sec,
