...
visualization_msgs::Marker ROSHelpers::CreateGenMarker(const double &x, const double &y, const double &z, const double &a, const double &r,
                                                       const double &g, const double &b, const double &scale, const int &id, const std::string &ns,
                                                       const int &type) {
...
void ROSHelpers::InitMarkers(const int &nMarkers, visualization_msgs::MarkerArray &centers, visualization_msgs::MarkerArray &dirs,
                             visualization_msgs::MarkerArray &text_info, visualization_msgs::MarkerArray &polygons,
...
void ROSHelpers::ConvertMatchingMarkers(const std::vector<std::pair<PlannerHNS::WayPoint, PlannerHNS::WayPoint>> &match_list,
    ...
    for (unsigned int i = 0; i < match_list.size(); i++) {
        visualization_msgs::Marker match_mkr =
            CreateGenMarker(0, 0, 0, 0, 1, 0, 0, 0.2, start_id + i, "matching_connections", visualization_msgs::Marker::LINE_STRIP);
...
int ROSHelpers::ConvertTrackedObjectsMarkers(const PlannerHNS::WayPoint &currState, const std::vector<PlannerHNS::DetectedObject> &trackedObstacles,
                                             visualization_msgs::MarkerArray &centers_d, visualization_msgs::MarkerArray &dirs_d,
                                             visualization_msgs::MarkerArray &text_info_d, visualization_msgs::MarkerArray &polygons_d,
                                             visualization_msgs::MarkerArray &tracked_traj_d, visualization_msgs::MarkerArray &centers,
                                             visualization_msgs::MarkerArray &dirs, visualization_msgs::MarkerArray &text_info,
                                             visualization_msgs::MarkerArray &polygons, visualization_msgs::MarkerArray &tracked_traj) {
    ...
    for (unsigned int i = 0; i < trackedObstacles.size(); i++) {
        ...
        visualization_msgs::Marker center_mkr =
            CreateGenMarker(trackedObstacles.at(i).center.pos.x, trackedObstacles.at(i).center.pos.y, trackedObstacles.at(i).center.pos.z,
        ...
        if (trackedObstacles.at(i).bDirection) {
            visualization_msgs::Marker dir_mkr = CreateGenMarker(trackedObstacles.at(i).center.pos.x, trackedObstacles.at(i).center.pos.y,
                                                                 trackedObstacles.at(i).center.pos.z + 0.5, trackedObstacles.at(i).center.pos.a, 0, 1,
                                                                 0, 0.3, centers.markers.size() + i, "Directions", visualization_msgs::Marker::ARROW);
            dir_mkr.scale.x = 0.4;
        ...
        //    else
        text_mkr = CreateGenMarker(trackedObstacles.at(i).center.pos.x + 0.5, trackedObstacles.at(i).center.pos.y + 0.5,
                                   trackedObstacles.at(i).center.pos.z + 1, trackedObstacles.at(i).center.pos.a, 1, 1, 1, 1.2,
        ...
        // Polygons
        visualization_msgs::Marker poly_mkr = CreateGenMarker(0, 0, 0, 0, 1, 0.25, 0.25, 0.1, centers.markers.size() * 3 + i, "detected_polygons",
        ...
        // Trajectories
        visualization_msgs::Marker traj_mkr =
            CreateGenMarker(0, 0, 0, 0, 1, 1, 0, 0.1, centers.markers.size() * 4 + i, "tracked_trajectories", visualization_msgs::Marker::LINE_STRIP);
...
void ROSHelpers::CreateCircleMarker(const PlannerHNS::WayPoint &_center, const double &radius, const int &start_id,
...
void ROSHelpers::ConvertPredictedTrqajectoryMarkers(std::vector<std::vector<PlannerHNS::WayPoint>> &paths,
                                                    visualization_msgs::MarkerArray &path_markers, visualization_msgs::MarkerArray &path_markers_d) {
    ...
    for (unsigned int i = 0; i < paths.size(); i++) {
        ...
        //    }
        //
        //    visualization_msgs::Marker path_mkr = CreateGenMarker(0,0,0,0,r,g,b,0.1,i,"Predicted_Trajectories",
        //    visualization_msgs::Marker::LINE_STRIP);

        visualization_msgs::Marker path_mkr =
            CreateGenMarker(0, 0, 0, 0, 1.0 * prop, 0.1 * prop, 0.1 * prop, 0.1, i, "Predicted_Trajectories", visualization_msgs::Marker::LINE_STRIP);
...
void ROSHelpers::ConvertCurbsMarkers(const std::vector<PlannerHNS::DetectedObject> &curbs, visualization_msgs::MarkerArray &curbs_markers,
    ...
    for (unsigned int i = 0; i < curbs.size(); i++) {
        if (curbs.at(i).contour.size() > 0) {
            visualization_msgs::Marker curb_mkr =
                CreateGenMarker(curbs.at(i).contour.at(0).x, curbs.at(i).contour.at(0).y, curbs.at(i).contour.at(0).z, 0, 1, 0.54, 0, 0.2, i,
...
void ROSHelpers::ConvertCollisionPointsMarkers(const std::vector<PlannerHNS::WayPoint> &col_points,
    ...
    for (unsigned int i = 0; i < col_points.size(); i++) {
        visualization_msgs::Marker mkr = CreateGenMarker(col_points.at(i).pos.x, col_points.at(i).pos.y, col_points.at(i).pos.z, 0, 1, 0, 0, 0.5, i,
                                                         "collision_points_rviz", visualization_msgs::Marker::SPHERE);
...
void ROSHelpers::ConvertParticles(std::vector<PlannerHNS::WayPoint> &points, visualization_msgs::MarkerArray &part_mkrs,
    ...
    for (unsigned int i = 0; i < points.size(); i++) {
        ...
        if (points.at(i).bDir == PlannerHNS::STANDSTILL_DIR)
            mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(i).pos.a, 1, 0, 0, 0.05, i, "Particles",
                                  visualization_msgs::Marker::ARROW);
        else if (points.at(i).bDir == PlannerHNS::FORWARD_DIR)
            mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(i).pos.a, 1, 1, 1, 0.05, i, "Particles",
                                  visualization_msgs::Marker::ARROW);
        else if (points.at(i).bDir == PlannerHNS::FORWARD_RIGHT_DIR)
            mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(i).pos.a, 0, 1, 0, 0.05, i, "Particles",
                                  visualization_msgs::Marker::ARROW);
        else if (points.at(i).bDir == PlannerHNS::FORWARD_LEFT_DIR)
            mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(i).pos.a, 0, 0, 1, 0.05, i, "Particles",
                                  visualization_msgs::Marker::ARROW);
        else if (points.at(i).bDir == PlannerHNS::BACKWARD_DIR)
            mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(i).pos.a, 1, 0, 1, 0.05, i, "Particles",
                                  visualization_msgs::Marker::ARROW);
        else
            mkr = CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y, points.at(i).pos.z, points.at(i).pos.a, 1, 1, 0, 0.05, i, "Particles",
                                  visualization_msgs::Marker::ARROW);
...
void ROSHelpers::TrajectoriesToMarkers(const std::vector<std::vector<std::vector<PlannerHNS::WayPoint>>> &paths,
...
void ROSHelpers::TrajectoriesToColoredMarkers(const std::vector<std::vector<PlannerHNS::WayPoint>> &paths,
...
void ROSHelpers::ConvertFromPlannerObstaclesToAutoware(const PlannerHNS::WayPoint &currState,
    ...
    for (unsigned int i = 0; i < trackedObstacles.size(); i++) {
        ...
        direction_marker.pose.orientation =
            tf::createQuaternionMsgFromYaw(UtilityHNS::UtilityH::SplitPositiveAngle(trackedObstacles.at(i).center.pos.a));
...
void ROSHelpers::VisualizeBehaviorState(const PlannerHNS::WayPoint &currState, const PlannerHNS::BehaviorState &beh, const bool &bGreenLight,
                                        const int &avoidDirection, visualization_msgs::Marker &behaviorMarker, std::string ns, double size_factor) {
...
void ROSHelpers::ConvertFromAutowareBoundingBoxObstaclesToPlannerH(const jsk_recognition_msgs::BoundingBoxArray &detectedObstacles,
    ...
    for (unsigned int i = 0; i < detectedObstacles.boxes.size(); i++) {
        ...
        obj.center = PlannerHNS::WayPoint(detectedObstacles.boxes.at(i).pose.position.x, detectedObstacles.boxes.at(i).pose.position.y, 0, 0);
...
void ROSHelpers::ConvertFromAutowareCloudClusterObstaclesToPlannerH(const PlannerHNS::WayPoint &currState, const double &car_width,
                                                                    ...
                                                                    vector<PlannerHNS::DetectedObject> &obstacles_list, const double max_obj_size,
...
void ROSHelpers::createGlobalLaneArrayMarker(std_msgs::ColorRGBA color, const autoware_msgs::LaneArray &lane_waypoints_array,
...
void ROSHelpers::GetTrafficLightForVisualization(std::vector<PlannerHNS::TrafficLight> &lights, visualization_msgs::MarkerArray &markerArray) {
    ...
    for (unsigned int i = 0; i < lights.size(); i++) {
        if (lights.at(i).lightState == RED_LIGHT) {
            visualization_msgs::Marker mkr = CreateGenMarker(lights.at(i).pos.x, lights.at(i).pos.y, lights.at(i).pos.z, 0, 1, 0, 0, 3, i,
                                                             "traffic_light_visualize", visualization_msgs::Marker::SPHERE);
            markerArray.markers.push_back(mkr);
        } else if (lights.at(i).lightState == GREEN_LIGHT) {
            visualization_msgs::Marker mkr = CreateGenMarker(lights.at(i).pos.x, lights.at(i).pos.y, lights.at(i).pos.z, 0, 0, 1, 0, 3, i,
                                                             "traffic_light_visualize", visualization_msgs::Marker::SPHERE);
            markerArray.markers.push_back(mkr);
        }
    }
...
void ROSHelpers::ConvertFromAutowareDetectedObjectToOpenPlannerDetectedObject(const autoware_msgs::DetectedObject &det_obj,
                                                                              PlannerHNS::DetectedObject &obj) {
...
void ROSHelpers::ConvertFromOpenPlannerDetectedObjectToAutowareDetectedObject(const PlannerHNS::DetectedObject &det_obj, const bool &bSimulationMode,
...
void ROSHelpers::UpdateRoadMap(const AutowareRoadNetwork &src_map, PlannerHNS::RoadNetwork &out_map) {
    ...
    PlannerHNS::MappingHelpers::ConstructRoadNetworkFromROSMessage(lanes, points, dts, inters, areas, line_data, stop_line_data, signal_data,
                                                                   vector_data, curb_data, roadedge_data, way_area, crossing, nodes_data, conn_data,
                                                                   origin, out_map);
}

void ROSHelpers::GetIndicatorArrows(const PlannerHNS::WayPoint &center, const double &width, const double &length,
                                    const PlannerHNS::LIGHT_INDICATOR &indicator, const int &id, visualization_msgs::MarkerArray &markerArray) {
    ...
    visualization_msgs::Marker mkr_l =
        PlannerHNS::ROSHelpers::CreateGenMarker(top_left.x, top_left.y, top_left.z, top_left.a, color_l.r, color_l.g, color_l.b, 1.0, id,
                                                "simu_car_indicator_left", visualization_msgs::Marker::ARROW);
    mkr_l.scale.y = 0.4;
    mkr_l.scale.z = 0.4;
    visualization_msgs::Marker mkr_r =
        PlannerHNS::ROSHelpers::CreateGenMarker(top_right.x, top_right.y, top_right.z, top_right.a, color_r.r, color_r.g, color_r.b, 1.0, id,
                                                "simu_car_indicator_right", visualization_msgs::Marker::ARROW);
    mkr_r.scale.y = 0.4;
    mkr_r.scale.z = 0.4;
