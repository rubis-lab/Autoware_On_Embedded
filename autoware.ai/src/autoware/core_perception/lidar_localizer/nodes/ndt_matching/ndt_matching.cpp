...
static pose initial_pose, predict_pose, predict_pose_imu, predict_pose_odom, predict_pose_imu_odom, previous_pose, previous_gnss_pose, ndt_pose,
    current_pose, current_pose_imu, current_pose_odom, current_pose_imu_odom, localizer_pose;
...
static double current_gnss_velocity = 0.0, current_gnss_angular_velocity = 0.0;
...
static void param_callback(const autoware_config_msgs::ConfigNDT::ConstPtr &input) {
    if (_use_gnss != input->init_pos_gnss) {
        init_pos_set = 0;
    } else if (_use_gnss == 0 && (initial_pose.x != input->x || initial_pose.y != input->y || initial_pose.z != input->z ||
...
static void gnss_pose_callback(const geometry_msgs::PoseStamped::ConstPtr &input) {
...
static void gnss_vel_callback(const geometry_msgs::TwistStamped::ConstPtr &input) {
    current_gnss_velocity = input->twist.linear.x;
    current_gnss_angular_velocity = input->twist.angular.z;
}

static void svl_callback(const rubis_msgs::PointCloud2::ConstPtr &points_msg, const rubis_msgs::PoseTwistStamped::ConstPtr &pose_twist_msg) {
    rubis::start_task_profiling();

    rubis::instance_ = points_msg->instance;
    rubis::lidar_instance_ = 0;

    tf::Quaternion gnss_q(pose_twist_msg->pose.pose.orientation.x, pose_twist_msg->pose.pose.orientation.y, pose_twist_msg->pose.pose.orientation.z,
                          pose_twist_msg->pose.pose.orientation.w);
    tf::Matrix3x3 gnss_m(gnss_q);

    current_gnss_pose.x = pose_twist_msg->pose.pose.position.x;
    current_gnss_pose.y = pose_twist_msg->pose.pose.position.y;
    current_gnss_pose.z = pose_twist_msg->pose.pose.position.z;
    gnss_m.getRPY(current_gnss_pose.roll, current_gnss_pose.pitch, current_gnss_pose.yaw);

    previous_gnss_pose = current_gnss_pose;

    current_gnss_velocity = pose_twist_msg->twist.twist.linear.x;
    current_gnss_angular_velocity = pose_twist_msg->twist.twist.angular.z;
...
static double wrapToPmPi(const double a_angle_rad) { return wrapToPm(a_angle_rad, M_PI); }
...
static inline void ndt_matching(const sensor_msgs::PointCloud2::ConstPtr &input) {
    if (_use_svl_gnss) {
        ...
        current_pose = current_gnss_pose;
        ndt_pose = current_gnss_pose;

        current_velocity = current_gnss_velocity;
        ...
        ndt_pose_msg.pose.position.x = current_gnss_pose.x;
        ndt_pose_msg.pose.position.y = current_gnss_pose.y;
        ndt_pose_msg.pose.position.z = current_gnss_pose.z;
        ndt_pose_msg.pose.orientation.x = gnss_q.x();
    ...
    if (_is_init_match_finished == false) {
        ...
        if (previous_score < _init_match_threshold)
            match_cnt--;
        else
            match_cnt = 10;
    ...
    mat_l.setValue(static_cast<double>(t(0, 0)), static_cast<double>(t(0, 1)), static_cast<double>(t(0, 2)), static_cast<double>(t(1, 0)),
                   static_cast<double>(t(1, 1)), static_cast<double>(t(1, 2)), static_cast<double>(t(2, 0)), static_cast<double>(t(2, 1)),
    ...
    mat_b.setValue(static_cast<double>(t2(0, 0)), static_cast<double>(t2(0, 1)), static_cast<double>(t2(0, 2)), static_cast<double>(t2(1, 0)),
                   static_cast<double>(t2(1, 1)), static_cast<double>(t2(1, 2)), static_cast<double>(t2(2, 0)), static_cast<double>(t2(2, 1)),
    ...
    if (!_is_matching_failed)
        _previous_success_score = previous_score;
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        tf::Quaternion b_l_q(m_base_to_localizer.getRotation().x(), m_base_to_localizer.getRotation().y(), m_base_to_localizer.getRotation().z(),
    ...
    std::cout << "(tf_x,tf_y,tf_z,tf_roll,tf_pitch,tf_yaw): (" << _tf_x << ", " << _tf_y << ", " << _tf_z << ", " << _tf_roll << ", " << _tf_pitch
              << ", " << _tf_yaw << ")" << std::endl;
    std::cout << "-----------------------------------------------------------------" << std::endl;
    ...
    private_nh.param<std::string>(node_name + "/task_response_time_filename", task_response_time_filename,
                                  "~/Documents/profiling/response_time/ndt_matching.csv");
    ...
    estimate_twist_pub = nh.advertise<geometry_msgs::TwistStamped>("/localizer_velocity", 10);
    ...
    // ros::Subscriber gnss_pose_sub = nh.subscribe("gnss_pose", 10, gnss_pose_callback);
    // ros::Subscriber gnss_vel_sub = nh.subscribe("gnss_vel", 10, gnss_vel_callback);
