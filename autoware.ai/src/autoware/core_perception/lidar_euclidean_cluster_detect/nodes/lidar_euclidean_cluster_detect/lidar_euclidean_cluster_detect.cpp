...
void transformBoundingBox(
    ...
    try {
        _transform_listener->transformPose(in_target_frame, ros::Time(),
                                           pose_in, in_header.frame_id,
                                           pose_out);
    } catch (tf::TransformException &ex) {
...
void publishDetectedObjects(
    ...
    rubis_detected_objects.lidar_instance = rubis::lidar_instance_;
...
void publishCloudClusters(const ros::Publisher *in_publisher,
    ...
    if (in_target_frame != in_header.frame_id) {
        ...
        for (auto i = in_clusters.clusters.begin();
             i != in_clusters.clusters.end(); i++) {
            ...
            try {
                _transform_listener->lookupTransform(in_target_frame,
                                                     ...
                                                     ros::Time(), *_transform);
                pcl_ros::transformPointCloud(in_target_frame, *_transform,
                                             i->cloud,
                                             cluster_transformed.cloud);
                _transform_listener->transformPoint(
                    in_target_frame, ros::Time(), i->min_point,
                    in_header.frame_id, cluster_transformed.min_point);
                _transform_listener->transformPoint(
                    in_target_frame, ros::Time(), i->max_point,
                    in_header.frame_id, cluster_transformed.max_point);
                _transform_listener->transformPoint(
                    in_target_frame, ros::Time(), i->avg_point,
                    in_header.frame_id, cluster_transformed.avg_point);
                _transform_listener->transformPoint(
                    in_target_frame, ros::Time(), i->centroid_point,
                    in_header.frame_id, cluster_transformed.centroid_point);
                ...
                if (_pose_estimation) {
                    ...
                    cluster_transformed.bounding_box.pose.orientation.w =
                        _initial_quat_w;
                }
...
void publishCentroids(const ros::Publisher *in_publisher,
    ...
    if (in_target_frame != in_header.frame_id) {
        ...
        for (auto i = centroids_transformed.points.begin();
            ...
            try {
                _transform_listener->transformPoint(
                    in_target_frame, ros::Time(), centroid_in,
                    in_header.frame_id, centroid_out);
...
clusterAndColorGpu(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr,
    ...
    std::vector<GpuEuclideanCluster::GClusterIndex> cluster_indices =
        _gecl_cluster.getOutput();

    unsigned int k = 0;

    for (auto it = cluster_indices.begin(); it != cluster_indices.end(); it++) {
        ClusterPtr cluster(new Cluster());
        cluster->SetCloud(in_cloud_ptr, it->points_in_cluster, _velodyne_header,
                          k, (int)_colors[k].val[0], (int)_colors[k].val[1],
...
void mergeClusters(const std::vector<ClusterPtr> &in_clusters,
    ...
    if (sum_cloud.points.size() > 0) {
        ...
        merged_cluster->SetCloud(
            ...
            (int)_colors[current_index].val[0],
            (int)_colors[current_index].val[1],
    ...
    for (size_t i = 0; i < in_clusters.size(); i++) {
        if (!visited_clusters[i]) {
            ...
            checkClusterMerge(i, in_clusters, visited_clusters, merge_indices,
                              in_merge_threshold);
            mergeClusters(in_clusters, out_clusters, merge_indices,
                          current_index++, merged_clusters);
        }
    }
    for (size_t i = 0; i < in_clusters.size(); i++) {
...
void segmentByDistance(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr,
    ...
    if (!_use_multiple_thres) {
        ...
        if (_use_gpu) {
            all_clusters =
                clusterAndColorGpu(cloud_ptr, out_cloud_ptr, in_out_centroids,
                                   _clustering_distance);
        } else {
            all_clusters =
                clusterAndColor(cloud_ptr, out_cloud_ptr, in_out_centroids,
                                _clustering_distance);
        }
#else
        all_clusters = clusterAndColor(cloud_ptr, out_cloud_ptr,
                                       in_out_centroids, _clustering_distance);
#endif
    } else {
        std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cloud_segments_array(
            5);
...
#ifdef GPU_CLUSTERING
            if (_use_gpu) {
                local_clusters = clusterAndColorGpu(
                    cloud_segments_array[i], out_cloud_ptr, in_out_centroids,
                    _clustering_distances[i]);
            } else {
                local_clusters =
    ...
    if (mid_clusters.size() > 0)
        checkAllForMerge(mid_clusters, final_clusters,
                         _cluster_merge_threshold);
    else
...
void velodyne_callback(const rubis_msgs::PointCloud2ConstPtr &in_sensor_cloud) {
    ...
    rubis::lidar_instance_ = in_sensor_cloud->instance;
    ...
    if (!_using_sensor_cloud) {
        ...
        if (_use_diffnormals)
            differenceNormalsSegmentation(nofloor_cloud_ptr,
        ...
        publishCloudClusters(&_pub_clusters_message, cloud_clusters,
                             _output_frame, _velodyne_header);
...
int main(int argc, char **argv) {
    ...
    private_nh.param<std::string>(node_name + "/task_response_time_filename",
                                  task_response_time_filename,
                                  "~/Documents/profiling/response_time/"
                                  "lidar_euclidean_cluster_detect.csv");
    ...
    output_cluster_str =
        std::string("/detection/lidar_detector/cloud_clusters_") + label;
    output_objects_str =
        std::string("/detection/lidar_detector/objects_") + label;
    rubis_output_objects_str =
        std::string("/detection/lidar_detector/rubis_objects_") + label;
    ...
    _pub_cluster_cloud =
        h.advertise<sensor_msgs::PointCloud2>(point_cluster_str.c_str(), 1);
    _pub_ground_cloud = h.advertise<sensor_msgs::PointCloud2>(
        cluster_centeroids_str.c_str(), 1);
    _centroid_pub =
        h.advertise<autoware_msgs::Centroids>(points_ground_str.c_str(), 1);
    _pub_points_lanes_cloud =
        h.advertise<sensor_msgs::PointCloud2>(output_lane_str.c_str(), 1);
    _pub_clusters_message = h.advertise<autoware_msgs::CloudClusterArray>(
        output_cluster_str.c_str(), 1);
    _pub_detected_objects = h.advertise<autoware_msgs::DetectedObjectArray>(
        output_objects_str.c_str(), 1);
    _pub_rubis_detected_objects = h.advertise<rubis_msgs::DetectedObjectArray>(
        rubis_output_objects_str.c_str(), 1);
    ...
    if (private_nh.getParam("use_diffnormals", _use_diffnormals)) {
        if (_use_diffnormals)
            ...
            ROS_INFO("Euclidean Clustering: Difference of Normals will not be "
                     "used.");
    }
    ...
    ROS_INFO("[%s] keep_lane_left_distance: %f", __APP_NAME__,
             _keep_lane_left_distance);
    private_nh.param("keep_lane_right_distance", _keep_lane_right_distance,
                     5.0);
    ROS_INFO("[%s] keep_lane_right_distance: %f", __APP_NAME__,
             _keep_lane_right_distance);
    private_nh.param("cluster_merge_threshold", _cluster_merge_threshold, 1.5);
    ROS_INFO("[%s] cluster_merge_threshold: %f", __APP_NAME__,
             _cluster_merge_threshold);
    ...
    ROS_INFO("[%s] clustering_distance: %f", __APP_NAME__,
             _clustering_distance);
    ...
    ROS_INFO("[%s] clustering_distances: %s", __APP_NAME__,
             str_distances.c_str());
    ...
    if (_use_multiple_thres) {
        ...
        if (distances_size == 0 || ranges_size == 0) {
            ROS_ERROR(
                "Invalid size of clustering_ranges or/and clustering_distance. \
