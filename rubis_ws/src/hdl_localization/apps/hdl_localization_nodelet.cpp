...
public:
  ...
  void onInit() override {
    ...
    points_sub = mt_nh.subscribe("/points_raw", 5, &HdlLocalizationNodelet::points_callback, this);
    globalmap_sub = nh.subscribe("/points_map", 1, &HdlLocalizationNodelet::globalmap_callback, this);
...
private:
  ...
  void gnsspose_callback(const geometry_msgs::PoseStampedConstPtr& pose_msg) {
    NODELET_INFO("initial pose received!!");
    std::lock_guard<std::mutex> lock(pose_estimator_mutex);
    gnsspose.pose = pose_msg->pose;
