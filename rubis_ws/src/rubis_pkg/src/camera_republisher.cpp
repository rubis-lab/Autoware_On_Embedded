...
void image_cb(const sensor_msgs::ImageConstPtr& msg){
    ...
    sensor_msgs::Image msg_with_intensity = *msg;
    
    msg_with_intensity.header.stamp = ros::Time::now();

    pub.publish(msg_with_intensity);

    rubis_msgs::Image rubis_msg_with_intensity;
...
int main(int argc, char** argv){
    ros::init(argc, argv, "camera_republisher");
    ros::NodeHandle nh;
    ...
    nh.param<std::string>(input_topic_name, input_topic, "/image_raw_origin");
    nh.param<std::string>(output_topic_name, output_topic, "/image_raw");

    sub = nh.subscribe(input_topic, 1, image_cb);      
    pub = nh.advertise<sensor_msgs::Image>(output_topic, 1);
    rubis_output_topic = "/rubis_"+output_topic.substr(1);
    pub_rubis = nh.advertise<rubis_msgs::Image>(rubis_output_topic, 1);
    
    // Scheduling & Profiling Setup
    std::string task_response_time_filename;
    private_nh.param<std::string>(node_name+"/task_response_time_filename", task_response_time_filename, "~/Documents/profiling/response_time/camera_republisher.csv");
