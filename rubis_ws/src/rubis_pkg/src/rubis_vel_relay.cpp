...
inline void relay(const geometry_msgs::TwistStampedConstPtr& msg){
...
void cb(const geometry_msgs::TwistStampedConstPtr& msg){
    rubis::instance_ = RUBIS_NO_INSTANCE;
    relay(msg);
}

void rubis_cb(const rubis_msgs::TwistStampedConstPtr& _msg){
...
int main(int argc, char* argv[]){
    ...
    nh.param<int>("/vel_relay/task_scheduling_flag", task_scheduling_flag, 0);
    nh.param<int>("/vel_relay/task_profiling_flag", task_profiling_flag, 0);
    nh.param<std::string>("/vel_relay/task_response_time_filename", task_response_time_filename, "~/Documents/profiling/response_time/vel_relay.csv");
    nh.param<int>("/vel_relay/rate", rate, 10);
    nh.param("/vel_relay/task_minimum_inter_release_time", task_minimum_inter_release_time, (double)10);
    nh.param("/vel_relay/task_execution_time", task_execution_time, (double)10);
    nh.param("/vel_relay/task_relative_deadline", task_relative_deadline, (double)10);
    nh.param<int>("/vel_relay/instance_mode", rubis::instance_mode_, 0);
    ...
    if(rubis::instance_mode_){
        ...
        rubis_pub_ = nh.advertise<rubis_msgs::TwistStamped>("/rubis_current_velocity", 10);
    }
    else{
        rubis_sub_ = nh.subscribe(input_topic_, 10, cb);
    }

    pub_ = nh.advertise<geometry_msgs::TwistStamped>("/current_velocity", 10);
