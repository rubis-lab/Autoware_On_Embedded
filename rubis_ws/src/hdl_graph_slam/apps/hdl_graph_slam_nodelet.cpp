...
public:
  ...
  virtual void onInit() {
    ...
    if(private_nh.param<bool>("enable_gps", true)) {
      ...
      inertiallabs_sub = mt_nh.subscribe("/Inertial_Labs/gps_data", 1024, &HdlGraphSlamNodelet::inertiallabs_callback, this);
    }
...
private:
  ...
  void inertiallabs_callback(const inertiallabs_msgs::gps_data::ConstPtr &inertiallabs_gps) {
    geographic_msgs::GeoPointStampedPtr gps_msg(new geographic_msgs::GeoPointStamped());
    gps_msg->header = inertiallabs_gps->header;
    gps_msg->position.latitude = inertiallabs_gps->LLH.x;
    gps_msg->position.longitude = inertiallabs_gps->LLH.y;
    gps_msg->position.altitude = inertiallabs_gps->LLH.z;
    gps_callback(gps_msg);
  }
...
private:
  ...
  ros::Subscriber inertiallabs_sub;
