...
LKF::LKF(){
    H_k = Matrix6f::Identity();

    Q_k = Matrix6f::Identity();

    R_k = Matrix6f::Identity();

    x_hat_k_prev = Vector6f::Zero();

    P_k_prev = Matrix6f::Identity();
}
...
Eigen::Vector6f LKF::run(float theta_t, Eigen::Vector6f u_k, Eigen::Vector6f z_k){ // u_k: control vector, z_k: observation vector
    // Prediction

    // Eigen::Matrix6f F_k = Matrix6f::Identity(); // Prediction Matrix
    // F_k(0,3) = theta_t;
    // F_k(1,4) = theta_t;
    // F_k(2,5) = theta_t; 

    // Eigen::Matrix6f B_k = Matrix6f::Identity() * theta_t; // Control Matrix
    // F_k(0,0) = F_k(0,0) * 0.5 * theta_t;
    // F_k(1,1) = F_k(1,1) * 0.5 * theta_t;
    // F_k(2,2) = F_k(2,2) * 0.5 * theta_t;
    ...
    /* yaw */
    x_hat_k(2) = x_hat_k_prev(2) + x_hat_k_prev(5) * theta_t + 0.5 * acc_yaw * pow(theta_t,2); 
    x_hat_k(2) = NormalizeRadian(x_hat_k(2), -1 * M_PI, M_PI);

    /* angular velocity */
    x_hat_k(5) = x_hat_k_prev(5) + acc_yaw * theta_t; 

    /* pose x, y */
    x_hat_k(0) = x_hat_k_prev(0) + vel * cos(x_hat_k_prev(2)) * theta_t + 0.5 * acc * cos(x_hat_k(2)) * pow(theta_t,2);
    x_hat_k(1) = x_hat_k_prev(1) + vel * sin(x_hat_k_prev(2)) * theta_t + 0.5 * acc * sin(x_hat_k(2)) * pow(theta_t,2);

    /* vel x, y */
    x_hat_k(3) = x_hat_k_prev(3) + acc * cos(x_hat_k(2)) * theta_t;
    x_hat_k(4) = x_hat_k_prev(4) + acc * sin(x_hat_k(2)) * theta_t;
